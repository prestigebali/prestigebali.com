...
export default async function HowToBookPage() {
  ...
  const steps = [
    ...
    { icon: <List className="w-12 h-12 text-primary" />, title: '2. Fill Out the Booking Form', description: 'Once you have selected a package, click the "Book Now" button. Fill out the form with your name, email, and phone number, then click "Submit Booking".' },
    { icon: <MessageCircle className="w-12 h-12 text-primary" />, title: '3. Contact Us (Optional)', description: 'For a faster response, you can contact us directly via WhatsApp or Email after submitting the form. Our team will assist you promptly.' },
    { icon: <CreditCard className="w-12 h-12 text-primary" />, title: '4. Confirmation & Payment', description: 'Our team will confirm the availability and details of your order. We will send you instructions for a secure and easy payment process.' },
    { icon: <CheckCircle className="w-12 h-12 text-primary" />, title: '5. Booking Complete!', description: 'Once payment is confirmed, your booking is complete! You will receive all your travel details and itinerary via email.' },
    { icon: <Plane className="w-12 h-12 text-primary" />, title: '6. Enjoy Your Holiday', description: 'Now you are ready for an unforgettable adventure. Our team will ensure everything runs smoothly from start to finish. Enjoy your holiday!' },
  ];
      ...
      <main className="flex-1">
        <section className="relative h-[50vh] w-full flex items-center justify-center text-center text-white bg-gray-800 pt-20">
          ...
          <div className="relative z-10 max-w-4xl mx-auto px-4">
            <h1 className="text-5xl md:text-6xl font-bold tracking-tight text-shadow-lg shadow-black/50">How to Book</h1>
            <p className="text-lg md:text-xl text-primary-foreground/90 max-w-3xl mx-auto mt-4 mb-8 text-shadow-md shadow-black/50">A step-by-step guide to planning your dream trip with us.</p>
          </div>
        </section>
              ...
              <div className="grid gap-12">
                {steps.map((step, index) => (
                  <div key={index} className="flex flex-col sm:flex-row items-start gap-6 sm:gap-8">
                    <div className="flex-shrink-0 flex items-center justify-center w-24 h-24 rounded-full bg-primary/10">{step.icon}</div>
                    <div>
                      <h3 className="text-2xl font-bold text-foreground mb-2">{step.title}</h3>
                      <p className="text-muted-foreground text-lg">{step.description}</p>
                      {step.link && <Button asChild className="mt-4"><Link href={step.link}>{step.linkLabel}</Link></Button>}
                    </div>
                  </div>
                ))}
              </div>
                    ...
                    <CardContent className="space-y-3 text-muted-foreground">
                      ...
                      {settings?.wiseEmail ? (
                        <a
                          ...
                          className="flex items-center gap-2 text-primary hover:underline font-medium text-lg break-all"
                        >
                          <ArrowLeftRight className="w-4 h-4 flex-shrink-0" />{settings.wiseEmail}
                        </a>
                      ) : <p className="text-lg">Not configured</p>}
